...
exports.Sha1 = exports.RANDOM_FACTOR = exports.MAX_VALUE_MILLIS = exports.FirebaseError = exports.ErrorFactory = exports.Deferred = exports.DBWrapper = exports.CONSTANTS = void 0;
...
var Preview = /*#__PURE__*/function () {
  function Preview(selector, options) {
    var _this = this;
    ...
    this.open.classList.add('btn');
    this.open.textContent = 'Open';
    this.close = document.createElement('button');
    this.close.classList.add('btn');
    this.close.classList.add('btn-primary');
    this.close.textContent = 'Upload';
  ...
  _createClass(Preview, [{
    ...
    key: "downloadPreview",
    value: function downloadPreview(e) {
      var closeBtn = this.container.querySelectorAll('.preview-close');
      var previewInfo = this.container.querySelectorAll('.preview-info');
      closeBtn.forEach(function (btn) {
        return btn.style.display = 'none';
      });
      previewInfo.forEach(function (info) {
        info.innerHTML = '';
        info.style.transform = 'translateY(0)';
        info.style.height = '20px';
        var progressBar = document.createElement('div');
        progressBar.classList.add('preview-bar');
        info.insertAdjacentElement('beforeend', progressBar);
      });
      this.onUpload(this.arrFiles, previewInfo);
    }
  }, {
    key: "showPreview",
    value: function showPreview(e) {
      ...
      this.open.insertAdjacentElement('afterend', this.container);
      var file = e.target.files;
      this.arrFiles = Array.from(file);

      if (!file.length) {
        return;
      }

      this.open.insertAdjacentElement('afterend', this.close);
      this.close.addEventListener('click', function (e) {
        return _this2.downloadPreview(e);
      });
      this.arrFiles.forEach(function (arrFile) {
        if (!arrFile.type.match('image')) {
          return;
        }
        ...
        reader.addEventListener('load', function (e) {
          ...
          var fileName = arrFile.name.split('.');
          previewContainer.innerHTML = "\n\t\t\t\t\t<div class=\"preview-close\" data-name=\"".concat(arrFile.name, "\">X</div>\n\t\t\t\t\t<img class=\"preview-img\" src=\"").concat(dataUrl, "\" alt=\"").concat(arrFile.name, "\">\n\t\t\t\t\t<div class=\"preview-info\">\n\t\t\t\t\t\t<div class=\"preview-name\">").concat(fileName[0], "</div>\n\t\t\t\t\t\t<div class=\"preview-bite\">").concat(_this2.bytesToSize(arrFile.size), "</div>\n\t\t\t\t\t</div>\n\t\t\t\t");

          _this2.container.insertAdjacentElement('beforeend', previewContainer);
        });
    ...
    key: "deletePreview",
    value: function deletePreview(e) {
      var deleteName = e.target;

      if (!deleteName.dataset.name) {
        return;
      }

      this.arrFiles = this.arrFiles.filter(function (file) {
        return file.name != deleteName.dataset.name;
      });
      ...
      var deleteItem = deleteName.closest('.preview-item');
      deleteItem.remove();
    }
  }, {
    key: "bytesToSize",
    ...
    value: function checkOptions() {
      ...
      if (this.type && Array.isArray(this.type)) {
        this.input.setAttribute('accept', this.type.join(','));
      }
    }
  }]);
...
var firebaseConfig = {
  apiKey: "AIzaSyCCPjDXzS-Je3ggL2nuqX8DUq6U0qkpEVc",
  authDomain: "image-pr.firebaseapp.com",
  projectId: "image-pr",
  storageBucket: "image-pr.appspot.com",
  messagingSenderId: "326248839668",
  appId: "1:326248839668:web:33f34daa552b84292da1a1"
};
var firebaseApp = (0, _app.initializeApp)(firebaseConfig);
var storage = (0, _storage.getStorage)(firebaseApp);
var imgPreview = new _upload.default('.file', {
  multiple: true,
  type: ['.png', '.jpg', '.jpeg', '.svg', '.gif', '.ico', '.webp'],
  onUpload: function onUpload(files, progress) {
    files.forEach(function (file, index) {
      var fileRef = (0, _storage.ref)(storage, "images/".concat(file.name));
      var task = (0, _storage.uploadBytesResumable)(fileRef, file);
      task.on('state_changed', function (snapshot) {
        var proceent = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0) + '%';
        console.log(proceent);
        var progressBar = progress[index].querySelector('.preview-bar');
        progressBar.style.width = proceent;
        progressBar.textContent = proceent;
      }, function (error) {
